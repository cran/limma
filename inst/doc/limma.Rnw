...
If you've installed the package and you're using Windows, simply type \texttt{library(limma)} at the R prompt then click on ``limma'' from the drop-down menu called ``Vignettes''.
If you're not using windows, you can type
\begin{Schunk}
\begin{Sinput}
> library(limma)
> browseURL(limmaguideURL())
\end{Sinput}
\end{Schunk}
or alternatively
\begin{Schunk}
...
\end{Sinput}
\end{Schunk}
and follow the links the limma package help.
This last option will also lead you to the printable pdf version of the User's Guide.
